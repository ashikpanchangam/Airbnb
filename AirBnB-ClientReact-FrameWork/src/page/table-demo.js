...
export class TableDemo extends React.Component {
  ...
  render() {
    ...
    if (!this.state.list.length) {
      return ( <Page actionBar={this.renderBreadcrumbs()} title='Hot Deals'>
      ...
      return (
        <Page actionBar={this.renderBreadcrumbs()} title='Hot Deals'>
          <Panel>
            <Griddle results={this.state.list} useGriddleStyles={false} columnMetadata={metaData} tableClassName="table"
                     showFilter={true}
                     showSettings={true} nextIconComponent={<button className="btn btn-info btn-xm">Next</button>}
                     columns={["image", "postDate", "title"]}/>
          </Panel>
        </Page>
      );
    }
